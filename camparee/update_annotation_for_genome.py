...
class UpdateAnnotationForGenomeStep(AbstractCampareeStep):
    ...
    def execute(self, sample, genome_indel_suffix, input_annot_file_path, chr_ploidy_file_path):
        ...
        ----------
        sample : Sample
            Sample for which to update annotation to match parental genome(s). If set to None,
            output/log paths will not include sample-specific directories.
        ...
        desired_chromosomes = self.chr_ploidy.keys()
        gender_index = None
        # Currently, pooled samples use reference without any variants
        self.genome_indel_file_path = None
        ...
        if sample:
            ...
            self.genome_indel_file_path = os.path.join(self.data_directory_path,
                                                       f"sample{sample.sample_id}",
                                                       CAMPAREE_CONSTANTS.GENOMEBUILDER_INDEL_FILENAME_PATTERN.format(genome_name=genome_indel_suffix))

        self.input_annot_file_path = input_annot_file_path
        self.updated_annot_file_path = os.path.join(self.data_directory_path,
        ...
        self.log_file_path = os.path.join(self.log_directory_path,
                                          f'sample{sample.sample_id}' if sample else "",
                                          CAMPAREE_CONSTANTS.UPDATEANNOT_LOG_FILENAME_PATTERN.format(genome_name=genome_indel_suffix))
    ...
    def get_validation_attributes(self, sample, genome_indel_suffix, input_annot_file_path, chr_ploidy_file_path):
        ...
        validation_attributes['sample_id'] = sample.sample_id if sample else None
    ...
    def _get_offsets_from_variant_file(genome_indel_filename):
        ...
        genome_indel_filename : string
            ...
            coordinate (numerical order). If None, return empty OrderedDict.
    ...
    def is_output_valid(validation_attributes):
        ...
        update_annot_outfile_path = os.path.join(data_directory,
                                                 f"sample{sample_id}" if sample_id else "",
                                                 CAMPAREE_CONSTANTS.UPDATEANNOT_OUTPUT_FILENAME_PATTERN.format(genome_name=genome_name))
        update_annot_logfile_path = os.path.join(log_directory,
                                                 f"sample{sample_id}" if sample_id else "",
                                                 CAMPAREE_CONSTANTS.UPDATEANNOT_LOG_FILENAME_PATTERN.format(genome_name=genome_name))
    ...
    def main():
        ...
        parser.add_argument('--sample', default=None,
                            help='String representation of a Sample object. Must provide '
                                 'this argument or "--sample_id". Set to "None" for sample-'
                                 'agnostic genome (overrides "--sample_id" argument).')
        ...
        if args.sample:
            ...
            sample = Sample(None, "debug", "", "", None)

        # Update sample with sample_id, if specified, and "None" not entered for
        # "--sample" argument.
        if args.sample_id and sample:
