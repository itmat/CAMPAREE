...
class GenomeBuilderStep(AbstractCampareeStep):
    ...
    def execute(self, sample, phased_vcf_file_path, chr_ploidy_data, reference_genome, build_ref_genome=False, chromosome_list=None):
        ...
        ----------
        sample : Sample
            Sample for which custom parental genomes will be generated. Ignored if
            build_ref_genome set to True.
        ...
        build_ref_genome : boolean
            Flag to construct a haploid, reference genome, filtered for the chromosomes
            contained in the ploidy file or chromosome_list argument (see below). The
            resulting genome file is saved in the general data directory and not a sample
            specific directory. This is currently used when samples were made from pooled
        ...
        # Variant file path is ignored (and potentially invalid) when building reference genome
        ...
        if build_ref_genome is False:
            ...
            # Build single reference genome with "_1" suffix (for compatibility with downstream steps)
            self.genome_names = [1]
            with open(self.log_file_path, 'a') as log_file:
                log_file.write('Building sample-agnostic reference genome.\n')
        
        # How the chromosome should be processed depends upon whether it is paired or not.
        for chromosome in self.chromosome_list:
            if build_ref_genome is True:
                self.make_reference_chromosome(chromosome)
            elif chromosome in self.unpaired_chr_list:
    ...
    def get_commandline_call(self, sample, phased_vcf_file_path, chr_ploidy_file_path, reference_genome_file_path, build_ref_genome=False, chromosome_list=None):
        """
        ...
        #steps. I'm leaving this as-is to maintain functionality with anyone else's
        ...
        if self.ignore_snps:
            command += " --ignore_snps"
        if build_ref_genome is True:
            command += " --build_reference_genome"
        if chromosome_list:
    ...
    def get_validation_attributes(self, sample, phased_vcf_file_path, chr_ploidy_file_path, reference_genome_file_path, build_ref_genome=False, chromosome_list=None):
        """
        ...
        validation_attributes['sample_id'] = sample.sample_id if build_ref_genome is False else None
        # When building reference genome, only constructed on genome with "_1" suffix
        validation_attributes['genome_names'] = self.genome_names if build_ref_genome is False else [1]
        validation_attributes['build_ref_genome'] = build_ref_genome
    ...
    def is_output_valid(validation_attributes):
        ...
        build_ref_genome = validation_attributes['build_ref_genome']

        valid_output = False

        genome_builder_logfile_path = os.path.join(log_directory,
                                                   f"sample{sample_id}" if build_ref_genome is False else "",
                                                   CAMPAREE_CONSTANTS.GENOMEBUILDER_LOG_FILENAME)
    ...
    def main():
        ...
        parser.add_argument('--sample', default=None,
                            help='String representation of a Sample object. Must provide '
                                 ...
                                 'or the "--build_reference_genome" argument.')
        ...
        parser.add_argument('-x', '--gender', action='store', choices=['male', 'female'], default=None,
                            help='Gender of input sample (male or female). Overrides gender from '
                                 'the "--sample" argument, if it is provided.')
        parser.add_argument('--build_reference_genome', action="store_true",
                            help='Build sample agnostic reference genome (no variants). '
                                 'Intended for samples made from pools of multiple individuals. '
                                 'If selected, genome builder will ignore any values provided for '
                                 ' the "--sample", "--sample_id", and "--gender" arguments.')
        ...
        if args.sample_id and args.build_reference_genome is False:
            sample.sample_id = args.sample_id
        if args.gender and args.build_reference_genome is False:
        ...
        genome_builder.execute(sample=sample,
                               ...
                               build_ref_genome=args.build_reference_genome,
