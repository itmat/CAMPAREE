...
class MoleculeMakerStep(AbstractCampareeStep):
    ...
    def make_molecule(self, sample, rng):
        ...
        # Pick random allele based on the gene's allelic distribution. Pooled samples
        # don't have allele-specific data. Pull all transcripts from the same annotation
        # (defaults to index of 1).
        allele_number = rng.choice(self.genome_names, p=self.allelic_quant[gene]) if sample.pooled is False else 1
        ...
        # Even though pooled samples don't have alleles, still included in the transcript
        # IDs for compatibility with downstream analyses. All transcripts from pooled
        # samples will come from the first allele.
    ...
    def execute(self, sample, sample_data_directory, output_type, output_molecule_count, seed=None,
                molecules_per_packet=None, rng=None, genome_and_annot_dir=None):
        ...
        rng: numpy Generator
            ...
            uses default RNG
        genome_and_annot_dir : string
            [OPTIONAL] If provided, load genome and annotation files from this directory
            instead of the sample_data_directory. Currently required for samples
            created from pools of multiple individuals.
        """

        # Pooled samples use common genome/annotations with a "_1" suffix
        self.genome_names = [1,2] if sample.pooled is False else [1]
        annotation_directory = sample_data_directory if sample.pooled is False else genome_and_annot_dir
        ...
        with open(log_file_path, "w") as log_file:
            ...
            if sample.pooled is True:
                allele_quant_path = "NA - Pooled Sample"
            ...
            self.allelic_quant = self.load_allelic_quants(allele_quant_path) if sample.pooled is False else None
            ...
            # NEED TO FIGRE OUT HOW TO PASS REFERENCE ANNOTATIONS AND GENOMES. I
            # COULD BREAK OUT A SEPARATE ANNOTATIONS / GENOMES DIRECTORY ARGUMENT
            # THAT WILL MATCH THE SAMPLE DATA DIR FOR NON-POOLED SAMPLES (SO IT
            # WILL CONTINUE TO FUNCTION AS IT CURRENTLY DOES). FOR POOLED SAMPLES,
            # IT POINTS TO A COMMON DIRECTORY WHERE THESE ARE STORED (USING THE
            # SAME _1 SUFFIX CONVENTION TO MAINTAIN COMPATIBILITY WITH DOWNSTREAM
            # CODE). THIS CHANGE WOULD ALSO MAKE IT POSSIBLE TO SEPARATE SAMPLE DATA
            # FROM THE SAMPLE-SPECIFIC ANNOTATIONS AND REFERENCES IN THE FUTURE.
            self.annotations = \
                [self.load_annotation(os.path.join(annotation_directory,
                                                   self._PARENTAL_ANNOT_FILENAME_PATTERN.format(genome_name=genome_name)))
                    for genome_name in self.genome_names]
            self.genomes = \
                [read_fasta(os.path.join(annotation_directory,
    ...
    def get_commandline_call(self, sample, sample_data_directory,
        ...
        if genome_and_annot_dir:
            command += f" --genome_and_annot_dir {genome_and_annot_dir}"
    ...
    def get_validation_attributes(self, sample, sample_data_directory,
        ...
        molecules_per_packet : integer
            ...
            be positive, non-zero integer (this is not currently checked).
        genome_and_annot_dir : string
            [OPTIONAL] If provided, load genome and annotation files from this
            directory instead of the sample_data_directory. Currently required
            for samples created from pools of multiple individuals. [Note: this
    ...
    def main():
        ...
        parser.add_argument('-d', '--sample_data_directory', required=True,
                            help='Path to sample data directory. Input read from and output written to this directory.')
        ...
        parser.add_argument('--genome_and_annot_dir', default=None, required=False,
                            help='Load genome and annotation files from this '
                                 'directory, instead of from the sample data directory. '
        ...
        molecule_maker.execute(sample=sample,
                               ...
                               genome_and_annot_dir=args.genome_and_annot_dir)
